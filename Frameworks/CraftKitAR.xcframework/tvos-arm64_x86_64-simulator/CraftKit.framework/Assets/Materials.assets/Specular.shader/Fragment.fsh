...
vec3 getAmbientLightIrradiance( const in vec3 ambientLightColor ) {
	...
	float flipNormal = 1.0;
		...
		vec3 worldNormal = inverseTransformDirection( geometry.normal, viewMatrix );
			...
			vec3 queryVec = flipNormal * vec3( flipEnvMap * worldNormal.x, -worldNormal.y, worldNormal.z );
	...
	float flipNormal = 1.0;
		...
		float specularMIPLevel = getSpecularMIPLevel( blinnShininessExponent, maxMIPLevel );
			...
			vec3 queryReflectVec = flipNormal * vec3( flipEnvMap * reflectVec.x, -reflectVec.y, reflectVec.z );
...
IncidentLight directLight;
	...
	vec3 outgoingLight = reflectedLight.directDiffuse + reflectedLight.indirectDiffuse + reflectedLight.directSpecular + reflectedLight.indirectSpecular + totalEmissiveRadiance;
		...
		vec4 envColor = textureCube( envMap, flipNormal * vec3( flipEnvMap * reflectVec.x, -reflectVec.y, reflectVec.z ) );
